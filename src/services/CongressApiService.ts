 ...
 * It configures fetch with the base URL and API key, integrates rate limiting,
    ...
    try {
      ...
      const response = await fetch(url.toString(), {
        ...
        headers: {
          ...
          "User-Agent": "mcp-congress_gov-server/0.1.0",
        },
      });

      clearTimeout(timeoutId);

      // Log raw response for debugging (redact API key)
      const redactedUrl = url
        .toString()
        .replace(this.config.apiKey, "[REDACTED]");
      ...
      if (!response.ok) {
        ...
        logger.error(`Congress API Error: ${errorMessage}`, {
          url: redactedUrl,
        ...
        throw new ApiError(
          `Congress API request failed with status ${response.status}: ${errorMessage}`,
          response.status,
          responseData
        );
      }
      ...
      if (error instanceof TypeError && error.message.includes("fetch")) {
        throw new ApiError(
          `Congress API request failed: Network error`,
          0,
          error
        );
      }
      if (
        ...
        throw new ApiError(
          `Congress API request failed: Request timeout after ${this.timeout}ms`,
          0,
          error
        );
      }
